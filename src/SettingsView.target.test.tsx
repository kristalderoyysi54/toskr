...
describe("粘贴与隐私按使用目的分区", () => {
  ...
  it.each(["隐私与化名", "发送前隐私检查（仅本机文本检查）", "启用隐私检查", "提示级类别"])("搜索 %s 直接定位隐私页", (target) => {
    expect(targetSettingsPageForSearch(target)).toBe("privacy");
    expect(visible(render(target), "privacy")).toBe(true);
  });

  it("化名与检测类别仅在各自搜索命中时展开", () => {
    expect(render("隐私与化名")).toContain("启用可逆化名");
    expect(render("启用隐私检查")).not.toContain("启用可逆化名");
    expect(render("提示级类别")).toContain("提示级类别");
    expect(render("提示级类别")).toContain("高风险规则不能单独关闭");
  });
